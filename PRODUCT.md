...
Creadores, diseñadores y makers que tienen un logo, un dibujo a línea, una ilustración de color plano o pixel art en baja resolución y necesitan un SVG limpio para escalarlo, cortarlo o editarlo. Llegan con una imagen concreta y una frustración concreta: otros conversores les devuelven trazos con "picos" (dientes de sierra) donde había una línea limpia. Uso puntual, no diario; una sola pantalla.

## Product Purpose

Convertir imágenes raster a SVG de forma fiel: líneas suaves donde el original tenía líneas, colores planos apilados sin costuras, y un modo píxel exacto sin pérdida para pixel art. Éxito: el usuario descarga un SVG que, superpuesto al original, no muestra picos ni huecos, y ve un número de fidelidad que lo confirma.
...
Un archivo PNG, JPG, WebP, GIF o BMP arrastrado, pegado desde el portapapeles o elegido con un selector. Resultados en segundos; el ajuste automático puede tardar unos segundos más y muestra progreso. El SVG se descarga o se copia al portapapeles y se abre en Figma, Illustrator, Inkscape o software de corte. Fotos y degradados están fuera de alcance: se avisa y se posteriza.

## Capabilities and Constraints

- Modos: Auto, Líneas/logo, Color plano, Píxel exacto. Motores: Potrace (principal) y VTracer.
- Controles principales: Suavizado (afilado a suave), Umbral, Colores, botón Ajuste automático con progreso y cancelación. Sección Avanzado plegada: reescalado, desenfoque, tolerancia de curva, manchas mínimas, invertir, fondo, transparencia, capas, rejilla de píxel, parámetros vtracer, optimizar con SVGO.
...
- Avisos previstos: parece una foto, trazos finos que pueden romperse, demasiados rectángulos en modo píxel, reescalado limitado por memoria, entrada demasiado grande (> 4096²), motor no disponible.
...
- Terminología en español: trazo, picos, fidelidad, nodos, esquinas, capas apiladas, píxel exacto.
...
- Imágenes sintéticas generadas en código (círculo anti-aliasado, línea diagonal fina, glifo con agujero, tres formas planas, sprite 32×32, logo transparente, ruido tipo foto) en `src/dev/synth.ts`.
